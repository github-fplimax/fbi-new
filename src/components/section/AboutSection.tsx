    ...
    >
      ...
      <div className="bg-image flex items-center justify-center">
        <div className="gap-6 block sm:hidden z-20 text-white">
          {/* mobile */}
          <div className="grid pt-6 grid-cols-2 gap-x-6 items-center justify-center sm:w-full w-72">
            {/* Bagian atas pl-[60px] sm:pl-0 pl-10 sm:pl-0 */}
            {counters.slice(0, 2).map((counter, index) => (
              <div
                key={index}
                className="text-center mx-auto container flex flex-col items-center justify-center"
              >
                <p className="transition-all text-2xl sm:text-4xl font-bold">
                  {counter.current}+
                </p>
                <p className="text-sm font-medium sm:text-xl">{counter.name}</p>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-3 gap-x-6 py-8 items-center justify-center w-72">
            {/* Bagian bawah */}
